...
The definitions for the Neural Network fed into DSSTNE is represented in a Json Format. Comprehensive category/type catalogs are available in [CDL.txt](CDL.txt), [DDL.txt](DDL.txt), and [LDL.txt](LDL.txt). A sample LDL is given below.
```js
{
    "Version" : 0.8,
    "Name" : "2 Hidden Layer",
